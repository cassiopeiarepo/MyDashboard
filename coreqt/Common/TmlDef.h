...
public:
	...
	TmlDefTypeDef(const QString& _name, Kind _kind) : name(_name), kind(_kind) {}
	...
	void addEnumItem(QString& _item) {
		if (enumItems.indexOf(_item) >= 0) return;
		enumItems.append(QString(_item));
	}

	bool isEnumItem(QString& _item) {
		return (enumItems.indexOf(_item) >= 0);
	}
...
private:
	void defineSimpleType(const QString& _name);

	QList<TmlDefTypeDef*> type_defs;
	QList<TmlNodeDef*> node_defs;
};
