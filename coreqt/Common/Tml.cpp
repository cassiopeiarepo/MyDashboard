...
TmlNode::TmlNode() : end_line(-1), end_pos(-1), parent(NULL), empty_element(false) {
...
void TmlNode::skipWhitespace(TmlSource* source) {
	...
	while (source->ch == ' ' || source->ch == '\t' || source->ch == QChar(TmlSource::EOL)) {
		source->next();
	}
}
			...
			if (source->ch == '<') {
				while (source->ch == '<') {
					...
					if (source->forward() == '/') {
						...
						node->end_line = source->line;
						node->end_pos = source->pos;
...
int TmlNode::getNameEnd(QStringList& lines) {
	int i = pos + 2;
	QChar ch = lines[line][i];
...
int TmlNode::getNameEnd2(QStringList& lines) {
	int i = end_pos + 2;
	QChar ch = lines[end_line][i];
	int size = lines[end_line].size();
...
int TmlArgument::getArgEnd(QStringList& lines) {
	int i = pos;
	QChar ch = lines[line][i];
	int size = lines[line].size();
