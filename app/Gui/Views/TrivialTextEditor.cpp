...
Vector<TTELine*> TTEBuffer::stringToLines(QString& str) 
{
    Vector<TTELine*> result;
...
void TTEBuffer::setText(QString& str) {
    ...
    Vector<TTELine*> text_lines = stringToLines(str);
    ...
    if (text_lines.size() == 1) {
        ...
        if (str != "") {
            ...
            for (int i = 0; i < src_line->line.size(); i++) {
                dest_line->line.append(pos, src_line->line[i]);
                pos++;
            }
        ...
        for (int i = 1; i < text_lines.size(); i++) {
            ...
            lines.append(line_nr, dest_line);
            ...
            if (i == text_lines.size() - 1) {
                ...
                if (line_str != "") {
                    ...
                    for (int j = 0; j < src_line->line.size(); j++) {
                        dest_line->line.append(j, src_line->line[j]);
                        pos++;
                    }
...
void TTEBuffer::removeBlock(int& line_from, int& pos_from, int line_to, int pos_to) 
{
    ...
    if (line_from == line_to) {
        ...
        if (str != "") {
            ...
            for (int i = pos_to; i < getLineSize(line_to); i++)
            {
                dest_line->line.append(j, src_line->line[i]);
                j++;
            }
...
public:
    InsertStringHistoryItem(TrivialTextEditor* _editor, Range& _range, QString& _str) :
...
void TrivialTextEditor::insertString(int& line_nr, int& pos, QString& str) 
{
...
void TrivialTextEditor::onBufferAddLine(int line_nr) {
    buffer_line_size.append(line_nr, 0);
    int max = getClientHeight() - (height() - scrollbar_size);
    vscrollbar->setRange(0, max);
}
...
void TrivialTextEditor::setTextColor(const Place& color_to, QColor color) {
    ...
    if (color_to.line == color_from.line) {
        ...
        for (int line = color_from.line + 1; line < color_to.line - 1; line++) 
        {
